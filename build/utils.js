...
/*: Layout*/
{
  var overwrite = layoutB.map(function (l) {
    return l.i;
  });

  function inOverwrite(l
  /*: LayoutItem*/
  ) {
    return overwrite.find(function (i) {
      return i === l.id;
    });
  }

  var keep = layoutA.filter(function (l) {
    return !inOverwrite(l);
  });
  return [].concat(_toConsumableArray(keep), _toConsumableArray(layoutB));
