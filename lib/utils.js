...
export function mergeLayouts(layoutA: Layout, layoutB: Layout): Layout {
  const overwrite = layoutB.map(l => l.i)
  function inOverwrite(l: LayoutItem) {
    return overwrite.find(i => i === l.id)
  }
  const keep = layoutA.filter(l => !inOverwrite(l))
  return [...keep, ...layoutB]
}
